...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <div class="box-content">
        <?php the_content(); ?>
        <?php if(get_theme_mod('cricket_tournament_remove_tags',true) != ''){ 
            $tags = get_the_tags(); // Retrieve the post's tags
             custom_output_tags(); 
        }?>

        <?php if(get_theme_mod('cricket_tournament_remove_category',true) != ''){ 
            if(has_category()){ 
        ...
        <?php if( get_theme_mod( 'cricket_tournament_remove_comment',true) != ''){ 
        ...
        if ( is_singular( 'attachment' ) ) {
            ...
            the_post_navigation( array(
                'prev_text' => _x( '<span class="meta-nav">Published in</span><span class="post-title">%title</span>', 'Parent post link', 'cricket-tournament' ),
            ) );
        } elseif ( is_singular( 'post' ) ) {
            // Previous/next post navigation.
            the_post_navigation( array(
                'next_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Next:', 'cricket-tournament' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
                'prev_text' => '<span class="meta-nav" aria-hidden="true">' . __( 'Previous:', 'cricket-tournament' ) . '</span> ' .
                    '<span class="post-title">%title</span>',
            ) );
        } ?>
        <div class="clearfix"></div>
    </div>
    <?php if( get_theme_mod( 'cricket_tournament_remove_related_post',true) != ''){  ?>
